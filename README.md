...
One more boundary belongs beside that qualifier, because it is about the *adversary* rather than the channel. The guarantee above answers the **confused deputy**: the model works for you, and the danger is that attacker-controlled content reaches it and turns a privileged action against you. Against that adversary the session is tainted, so `BashExec` refuses the spawn outright. It does not answer a model that is itself hostile — abliterated, backdoored, or simply misaligned — because such a model's session is *clean by construction*: your own prompt is trusted, nothing has tainted it, so the lattice permits the spawn, and after that the lattice sees none of the shell's syscalls. The perimeter for that surface is the pod's netns default-deny, which is a real boundary but a narrower claim than non-interference. If the model is your adversary, remove the capability rather than track the flow: the **`untrusted-model`** profile does exactly that. See **[Posture for an adversarial model](docs/adversarial-model-posture.md)**.
...
> **Vendor neutrality.** The core library and the generic `credentials.env` / `PodSpec` interface are *intended* to be vendor-agnostic, but the surface is not fully clean today. Two things break it: (1) the *reference agent runner* shipped with `nucleus run`/`shell` is coupled to one specific assistant CLI (binary name, default model string, permission-bypass flag), and (2) `nucleus-spec` still hardcodes specific LLM-vendor hostnames and workload-identity defaults (see [Known Gaps](#known-gaps)). A PreToolUse hook for AI coding assistants lives in an external private orchestrator built on this runtime; the vendor-side half of that seam is public at [claude-code-nucleus](https://github.com/coproduct-opensource/claude-code-nucleus). Treat the runner as an integration *example*, not a vendor-agnostic component.
...
**Runaway agents:** lockdown can be entered three ways — an operator writing the signal file, a fleet command over the gRPC stream, and now the pod's own **behavioural circuit breaker**, which trips after `NUCLEUS_DENIAL_BUDGET` (default 10) *consecutive* denials. Consecutive is what makes it safe to leave on: any allow resets the run, so real work never accumulates toward it, while a run of refusals with nothing succeeding in between is an agent probing rather than working. All three set the same flag and deny through the same already-enforced gate; clearing it stays a human action. Set the budget to `0` to disable.
...
**Command output is classified, not assumed:** bytes an agent obtains by running a command are observed as an ingest **when the command could reach the network** — `curl`, `wget`, `nc`, `ssh`, a pipe into any of them, or an opaque interpreter form like `python -c`. Ordinary local work (`cargo test`, `ls`, `grep`) does not taint, which is what makes the protection affordable to leave on; blanket-tainting made a session "one privileged action then locked", which is why the old default was off entirely. The classifier is the same detector the egress gate uses, so a command flagged as egress is also tainted as ingress, and it is fail-closed on unparseable input. It is a heuristic over command text and obfuscated program names can evade it: the containment is the pod's default-deny egress, which stops the fetch rather than labelling it. `NUCLEUS_PARANOID_TOOL_IO=1` still blanket-taints all command output on both transports for operators who want no heuristic in the path.

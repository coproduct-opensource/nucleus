...
| **Kani** (bounded model checking) | 122 harnesses repo-wide (portcullis 70, portcullis-core 26, ck-kernel 17, nucleus-ifc-kernel 6, nucleus-econ-kernels 1, nucleus-permission-market 2) — census by `scripts/formal-numbers.sh` | DecisionToken linearity, lattice adjunction, flow-graph isolation, constitutional-kernel admission contract | `kani-nightly.yml` runs `cargo kani -p portcullis` (70) + `-p ck-kernel` (17). **Of ck-kernel's 17, only 5 have ever completed** — every harness that constructs a `BTreeSet<String>` fails to terminate, including the refinement bridge that would carry the other 12 (`KANI-STATUS.md`) |

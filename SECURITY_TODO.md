...
Status
- **CLOSED for the domain-separator half (2026-09-10); the conservation half remains deferred to the Tier-3 ADR.**
- `nucleus_lineage::canonical_sth_bytes` is now prefixed with `nucleus-lineage-sth/v1\n` (71 bytes). The old doc argued the separator was unnecessary because "the signature scope … is established out-of-band by the verifier knowing which kid maps to which log" — which is not a property the bytes carry, it is a hope about how they are used. The sibling `nucleus_verifier_service::signing::canonical_sth_bytes` already tags itself; now neither preimage can be read as the other, and `the_preimage_cannot_be_confused_with_the_other_sth_protocol` asserts it in both directions plus that the pre-#30 body is a prefix of neither.
- **The prescribed fix was not taken, because it would have removed a signed field.** The plan said to make `signed_note::checkpoint_signed_bytes` the default. That body covers `origin`, `tree_size` and `root_hash` — and **not `timestamp_ms`**, which the native preimage does cover and which `nucleus_envelope::verify` reads to reject stale anchors (`VerifyBundleError::StaleSth`, `verify.rs:994-1005`). Signing the C2SP body here would let anyone holding a validly-signed STH rewrite `timestamp_ms` to now and turn `sth_max_age` into a no-op. The existing test at `integration.rs:1456` demonstrates the coverage: it has to **re-sign** with the stale timestamp, which is only necessary because the timestamp is signed.
- C2SP keeps the role it already had: `CosignatureKind::{Nucleus, C2sp}` shows it was always a *second* protocol for federating with external witnesses, separately domain-separated by its `origin` line — not a replacement for the native STH signature.
- Migration is dual-accept, because `SignedTreeHead` has no version field and a signature cannot announce which preimage produced it. `SignedTreeHead::verify` tries the current form then `canonical_sth_bytes_legacy`; `nucleus_envelope::verify_bundle` does the same for `Nucleus`-kind cosignatures via a candidate list. Both fallbacks are marked for removal one release on, and the error surfaced on total failure is the *current* form's, so the message names the encoding a caller should be producing.
- Non-vacuity, both layers: `an_sth_signed_under_the_legacy_preimage_still_verifies` also asserts a signature over *neither* preimage is refused, so dual-accept widens the accepted set by exactly one encoding rather than becoming a no-op. `a_legacy_preimage_cosignature_still_counts_during_the_dual_accept_window` covers the envelope path. Perturbations run: removing the separator reds with `assertion failed: !ours.starts_with(&legacy[..])`; dropping the envelope's legacy candidate reds with `InsufficientCosignatures { verified: 0, required: 1 }` — the liveness break a missing fallback would cause.
- Still open, deferred to the Tier-3 ADR: the ~40 canonicalization functions in 7 encoding families, the five signed-tree-head layouts, and the conservation gap (`LedgerCore<Unit>`, `BudgetLattice::charge`'s second unreconciled account, `ck-types::BudgetBounds`).

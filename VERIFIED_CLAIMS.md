...
| VC-002 | **Complete mediation** — no side effect bypasses the kernel. Every I/O operation (filesystem, network, process spawn) is intercepted and evaluated by the Nucleus kernel before execution. | Specified | `crates/portcullis/src/kernel.rs`, `crates/portcullis/tests/kernel_executable_spec.rs` | `crates/nucleus-tool-proxy/src/mcp.rs` (MCP handlers), `crates/nucleus-tool-proxy/src/main.rs` (HTTP handlers) | — |
...
| VC-005 | **Trace completeness** — all mediated actions produce audit records. Every decision made by the kernel (allow, deny, attenuate) is recorded in a tamper-evident trace log. | Specified | `crates/portcullis/src/kernel.rs`, `crates/portcullis/tests/kernel_executable_spec.rs` | `crates/portcullis/src/kernel.rs` (append-only trace in `Kernel::decide`) | — |

...
| VC-002 | **Complete mediation** — no side effect bypasses the kernel. Every I/O operation (filesystem, network, process spawn) is intercepted and evaluated by the Nucleus kernel before execution. | Specified | `crates/portcullis/src/kernel.rs` — `Kernel::decide` covers all 12 operations; `crates/portcullis/tests/kernel_executable_spec.rs` — executable specification with 30+ scenarios | `crates/portcullis/src/kernel.rs`, `crates/nucleus-tool-proxy/src/mcp.rs`, `crates/nucleus-tool-proxy/src/main.rs` | — |

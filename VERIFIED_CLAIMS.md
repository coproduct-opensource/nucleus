...
| VC-002 | **Complete mediation** — no side effect bypasses the kernel. Every I/O operation (filesystem, network, process spawn) is intercepted and evaluated by the Nucleus kernel before execution. | Machine-proved | `crates/portcullis/src/kernel.rs` — `DecisionToken` linear type (sealed, non-Clone, non-Copy, `#[must_use]`); `crates/portcullis/src/kani.rs` — `proof_decision_token_unforgeable`, `proof_denied_ops_have_no_token`, `proof_token_operation_matches_decision`; Sandbox/Executor APIs require `&DecisionToken` for all I/O | `crates/portcullis/src/kernel.rs`, `crates/nucleus/src/sandbox.rs`, `crates/nucleus/src/command.rs` | CI-verified |

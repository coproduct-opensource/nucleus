//! Agent tool settings.json security scanner.
//!
//! Parses an agent tool's `settings.json` format (the `.<tool>/settings.json`
//! convention) and projects its permission rules onto the portcullis
//! CapabilityLattice for uninhabitable_state analysis.
...
/// Scan an agent tool's settings.json file for security issues.
pub fn scan_agent_settings(
    path: &Path,
) -> Result<(Vec<Finding>, AgentSettingsSummary), AuditError> {
    ...
    if let Some(perms) = perms {
        ...
        if state_risk == portcullis::StateRisk::Uninhabitable {
            findings.push(Finding {
                ...
                title: "Lethal uninhabitable_state in agent settings".to_string(),
            ...
            findings.push(Finding {
                ...
                category: "uninhabitable_state".to_string(),
                title: "Partial uninhabitable_state in agent settings".to_string(),
                description:
    ...
    let summary = AgentSettingsSummary {
...
/// Project an agent tool's allow/deny rules to a CapabilityLattice for uninhabitable_state analysis.
...
mod tests {
    ...
    fn test_uninhabitable_mitigated_by_deny() {
        ...
        let (findings, _) = scan_agent_settings(&path).unwrap();

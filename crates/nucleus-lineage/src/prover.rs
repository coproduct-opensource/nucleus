...
pub trait MerkleProver: Send + Sync {
    ...
    /// this prover will anchor to. The implementation should call its
    /// witness's `sign_sth` against the *current* root; callers should
    /// gather all needed inclusion proofs first, then call this once,
    /// to ensure every proof's parent tree matches `sth.tree_size`.

...
async fn prepare_for_test(
    ...
    pod_boot_identity::prepare(Inputs {
        state: st, pod_dir: dir, spec: &spec, image: &image, id,
        grant: &crate::net::IdentityGrant::Granted,
        vsock_path: socket, jail_owner: None,
        task_token: None, pod_certificate: None, broker_serve: serve,
    }).await
}
...
async fn host_spec_is_served_before_spawn_and_launch_error_releases_identity() {
    ...
    let manager = crate::identity::IdentityManager::new("test.local", std::time::Duration::from_secs(3600)).unwrap();
    st.identity_manager = Some(manager.clone());
    let id = uuid::Uuid::new_v4();
    let socket = dir.path().join("vsock");
    let ready = prepare_for_test(&st, dir.path(), id, &socket).await.unwrap();
    ...
    tokio::io::BufReader::new(stream).read_line(&mut response).await.unwrap();
    ...
    tokio::time::timeout(std::time::Duration::from_secs(5), async {
        while api.exists() || manager.get_attestation(&id.to_string()).await.is_some() { tokio::task::yield_now().await; }
    }).await.unwrap();
...
async fn unavailable_workload_api_refuses_before_vmm_spawn() {
    let dir = tempfile::tempdir_in("/tmp").unwrap();
    let mut st = state(&dir);
    st.identity_manager = Some(crate::identity::IdentityManager::new("test.local", std::time::Duration::from_secs(3600)).unwrap());
    let blocker = dir.path().join("file-not-directory");
    std::fs::write(&blocker, b"occupied").unwrap();
    let error = match prepare_for_test(&st, dir.path(), uuid::Uuid::new_v4(), &blocker.join("vsock")).await {
        Ok(_) => panic!("missing listener cannot authorize VMM spawn"),
        Err(error) => error,
    };
    assert!(error.to_string().contains("workload API must be ready before VMM spawn"), "{error}");
}

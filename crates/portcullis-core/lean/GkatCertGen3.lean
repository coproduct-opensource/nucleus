...
/-! # GkatCertGen3: emitted instance certificate (machine-generated; see emit_cert.py) -/

namespace GkatCertGen3
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh bT pA))
def fP : Exp Act Tst := (Exp.seq pA (Exp.wh bT pA))
...
def L1 : LevelG Act Tst := ([(bT, pA)], [], (Exp.test (BExp.not bT)))

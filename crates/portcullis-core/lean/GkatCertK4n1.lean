...
/-! # GkatCertK4n1: emitted instance certificate (machine-generated; see emit_cert.py) -/

namespace GkatCertK4n1
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh (BExp.not bT) pA)) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA)))
def fP : Exp Act Tst := (Exp.seq (Exp.ite bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.test bT)) (Exp.seq (Exp.wh (BExp.not bT) pA) (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA))) (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA))
...
def sol3 : Exp Act Tst := guardedFold [] (Exp.test BExp.one)
def sol2 : Exp Act Tst := guardedFold [(bT, Exp.seq pA sol3)] (Exp.test BExp.zero)
def L1 : LevelG Act Tst := ([((BExp.not bT), pA)], [(bT, Exp.seq pA sol2)], (Exp.test BExp.zero))
def sol1 : Exp Act Tst := levelSol L1
def sol0 : Exp Act Tst := guardedFold [((BExp.not bT), Exp.seq pA sol1), (bT, Exp.seq pA sol2)] (Exp.test BExp.zero)
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inr none => 0
  | .inr (some (Sum.inl (Sum.inr (Sum.inl ())))) => 1
  | .inr (some (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))) => 2
  | .inr (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))) => 2
  | .inr (some (Sum.inr (Sum.inr ()))) => 3
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl none =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨2, qstep_0_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
  ...
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl ())))))) a = bval W (QAut.hlt 1) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))))) a = bval W (QAut.hlt 2) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))) a = bval W (QAut.hlt 2) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inr (Sum.inr ()))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr ()))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl ())))) => exact (List.Mem.tail _ (List.Mem.head _))
    | Sum.inr (some (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
    | Sum.inr (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
    | Sum.inr (some (Sum.inr (Sum.inr ()))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
  onto_states := by
    intro q hq
    match q, hq with

...
/-! # GkatCertK4n3: emitted instance certificate (machine-generated; see emit_cert.py) -/

namespace GkatCertK4n3
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.test bT)) (Exp.seq (Exp.wh (BExp.not bT) pA) (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA))) (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA))
def fP : Exp Act Tst := (Exp.seq (Exp.wh (BExp.not bT) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA))) (Exp.ite bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA)) (Exp.test (BExp.not bT))))
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl none => 0
  | .inl (some (Sum.inl (Sum.inr (Sum.inl ())))) => 1
  | .inl (some (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))) => 2
  | .inl (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))) => 2
  | .inl (some (Sum.inr (Sum.inr ()))) => 3
  | .inr none => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl none =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl none) ()
                = some ((), (Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ())))))) := by rfl
            rw [hred] at hst
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ()))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
        | true =>
            rw [hb] at hst
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ())))))) a = bval W (QAut.hlt 1) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ()))))) ()
                = some ((), (Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ())))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
        ...
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ()))))) ()
  ...
  | Sum.inl (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))) a = bval W (QAut.hlt 2) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inl (some (Sum.inr (Sum.inr ()))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr ()))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ())))) => exact (List.Mem.tail _ (List.Mem.head _))
    | Sum.inl (some (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
    | Sum.inl (some (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
    | Sum.inl (some (Sum.inr (Sum.inr ()))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
  ...
  onto_states := by
    ...
    | 1, _ => exact ⟨(Sum.inl (some (Sum.inl (Sum.inr (Sum.inl ()))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inl (Sum.inr (Sum.inl ()))))))))), rfl⟩
    ...
    | 3, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr ())))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr ())))))))), rfl⟩

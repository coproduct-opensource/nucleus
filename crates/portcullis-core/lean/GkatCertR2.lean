...
/-! # GkatCertR2: emitted instance certificate (machine-generated; see emit_cert.py) -/

namespace GkatCertR2
...
/-! ## Ring solutions.  2-ring {s1,s2}, halt at s2 on `¬t`; header s2.  s1's `¬t` exit
    goes to s3 whose solution is the pure test `¬t?`, so the exit is INLINED into the
    body and falls out through the header's own exit test.  s0 is straight-line. -/

def sol3 : Exp Act Tst := .test (.not bT)
/-- s1's factor: on `¬t` the exit `p ; sol3` inlined; on `t` the ring action to the
    header s2. -/
def F1 : Exp Act Tst := .ite (.not bT) (.seq pA (.test (.not bT))) pA
def BODY : Exp Act Tst := .seq pA F1
def sol2 : Exp Act Tst := .seq (.wh bT BODY) (.test (.not bT))
def sol1 : Exp Act Tst := .seq F1 sol2
def sol0 : Exp Act Tst :=
  .ite (.not bT) (.seq pA sol3) (.ite bT (.seq pA sol1) (.test BExp.zero))

private theorem habs2 : EquivBA (.seq (.test (.not bT)) sol2) (.test (.not bT)) :=
  test_header_absorb (.not bT) bT BODY (himp_self (.not bT))

theorem ring_E2 : EquivBA sol2 (.ite bT (.seq pA sol1) (.test (.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (.not bT))))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA F1 sol2)) (EquivBA.base (Equiv.refl _)))

theorem ring_E1 : EquivBA sol1
    (.ite (.not bT) (.seq pA sol3) (.ite bT (.seq pA sol2) (.test BExp.zero))) :=
  EquivBA.trans
    (EquivBA.symm (EquivBA.base (Equiv.u5 (.not bT) (.seq pA (.test (.not bT))) pA sol2)))
    (EquivBA.trans
      (EquivBA.ite_c
        (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.test (.not bT)) sol2))
          (EquivBA.seq_c (EquivBA.base (Equiv.refl pA)) habs2))
...
theorem ring_E3 : EquivBA sol3 (.test (.not bT)) :=
  EquivBA.base (Equiv.refl _)

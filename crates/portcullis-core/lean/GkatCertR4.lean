...
/-! # GkatCertR4: emitted instance certificate (machine-generated; see emit_cert.py) -/

namespace GkatCertR4
...
def eP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.seq pA pA) (Exp.wh bT pA)) pA)))
def fP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.wh bT pA)) pA)))
...
/-! ## Ring solutions.  2-ring {s0,s1}, halt at s0 on `¬t`; header s0.  s1's `t` exit
    runs through s2 into the tail loop s3, which terminates with halt guard `¬t` — so
    the whole exit continuation is INLINED into the body and falls out through the
    header's exit test. -/

def sol3 : Exp Act Tst := .seq (.wh bT pA) (.test (.not bT))
def sol2 : Exp Act Tst := .seq pA sol3
/-- s1's factor: on `t` the exit `p ; sol2` inlined; on `¬t` the ring action back to
    the header s0. -/
def F1 : Exp Act Tst := .ite bT (.seq pA sol2) pA
def BODY : Exp Act Tst := .seq pA F1
def sol0 : Exp Act Tst := .seq (.wh bT BODY) (.test (.not bT))
def sol1 : Exp Act Tst := .seq F1 sol0

private theorem habs0 : EquivBA (.seq (.test (.not bT)) sol0) (.test (.not bT)) :=
  test_header_absorb (.not bT) bT BODY (himp_self (.not bT))

theorem ring_E3 : EquivBA sol3 (.ite bT (.seq pA sol3) (.test (.not bT))) :=
  EquivBA.base (salomaa_solution_exists bT pA (.test (.not bT)))

theorem ring_E2 : EquivBA sol2
    (.ite (.not bT) (.seq pA sol3) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  both_arms (.not bT) bT (.seq pA sol3) (himp_dneg bT)
...
theorem ring_E0 : EquivBA sol0 (.ite bT (.seq pA sol1) (.test (.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (.not bT))))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA F1 sol0)) (EquivBA.base (Equiv.refl _)))

theorem ring_E1 : EquivBA sol1
    (.ite (.not bT) (.seq pA sol0) (.ite bT (.seq pA sol2) (.test BExp.zero))) :=
  EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 bT (.seq pA sol2) pA sol0)))
    (EquivBA.trans
      (EquivBA.ite_c
        (EquivBA.trans (EquivBA.base (Equiv.s1 pA sol2 sol0))
          (EquivBA.seq_c (EquivBA.base (Equiv.refl pA))
            (EquivBA.trans (EquivBA.base (Equiv.s1 pA sol3 sol0))
              (EquivBA.seq_c (EquivBA.base (Equiv.refl pA))
                (EquivBA.trans (seq_assoc (.wh bT pA) (.test (.not bT)) sol0)
                  (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) habs0))))))

...
/-! # GkatCertR6: emitted instance certificate (machine-generated; see emit_cert.py) -/

namespace GkatCertR6
...
def eP : Exp Act Tst := (Exp.wh bT (Exp.seq (Exp.seq (Exp.seq pA pA) (Exp.wh (BExp.not bT) pA)) (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA))))
def fP : Exp Act Tst := (Exp.wh bT (Exp.seq (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.wh (BExp.not bT) pA)) (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA))))
...
/-! ## Ring solutions.  4-ring s0 -t-> s1 -> s2 (self on `¬t`) -t-> s3 -t-> s0, halts
    at s0/s3 on `¬t`; header s0, park at s3. -/

/-- s3's park: halt on `¬t`, else the ring action to the header s0. -/
def PKD : Exp Act Tst := .ite (.not bT) (.test (.not bT)) pA
/-- s2's self-loop. -/
def SLN : Exp Act Tst := .wh (.not bT) pA
def BODY : Exp Act Tst := .seq pA (.seq pA (.seq SLN (.seq pA PKD)))
def sol0 : Exp Act Tst := .seq (.wh bT BODY) (.test (.not bT))
def sol3 : Exp Act Tst := .seq PKD sol0
def sol2 : Exp Act Tst := .seq SLN (.seq pA sol3)
def sol1 : Exp Act Tst := .seq pA sol2

private theorem habs0 : EquivBA (.seq (.test (.not bT)) sol0) (.test (.not bT)) :=
  test_header_absorb (.not bT) bT BODY (himp_self (.not bT))

theorem ring_E0 : EquivBA sol0 (.ite bT (.seq pA sol1) (.test (.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (.not bT))))
    (EquivBA.ite_c
      (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq pA (.seq SLN (.seq pA PKD))) sol0))
        (EquivBA.seq_c (EquivBA.base (Equiv.refl pA))
          (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq SLN (.seq pA PKD)) sol0))
            (EquivBA.seq_c (EquivBA.base (Equiv.refl pA))
              (EquivBA.trans (EquivBA.base (Equiv.s1 SLN (.seq pA PKD) sol0))
                (EquivBA.seq_c (EquivBA.base (Equiv.refl SLN))
                  (EquivBA.base (Equiv.s1 pA PKD sol0))))))))
      (EquivBA.base (Equiv.refl _)))

theorem ring_E1 : EquivBA sol1
    (.ite (.not bT) (.seq pA sol2) (.ite bT (.seq pA sol2) (.test BExp.zero))) :=
  both_arms (.not bT) bT (.seq pA sol2) (himp_dneg bT)

theorem ring_E2 : EquivBA sol2
    (.ite (.not bT) (.seq pA sol2) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (.not bT) pA (.seq pA sol3)))
    (else_expand (.not bT) bT (himp_dneg bT))

theorem ring_E3 : EquivBA sol3 (.ite bT (.seq pA sol0) (.test (.not bT))) :=
  EquivBA.trans (park_solves (.not bT) pA sol0 habs0)
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inr (some (Sum.inl (Sum.inr ()))) => 2
  | .inr (some (Sum.inr (Sum.inl (Sum.inr ())))) => 3
  | .inr (some (Sum.inr (Sum.inr (Sum.inr ())))) => 0
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inr (some (Sum.inl (Sum.inr ()))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr ()))))) a = bval W (QAut.hlt 2) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inr ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr (Sum.inr ())))))) a = bval W (QAut.hlt 0) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inr (some (Sum.inl (Sum.inr ()))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
    | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr ())))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inr (some (Sum.inr (Sum.inr (Sum.inr ())))) => exact (List.Mem.head _)
  onto_states := by
    intro q hq
    match q, hq with

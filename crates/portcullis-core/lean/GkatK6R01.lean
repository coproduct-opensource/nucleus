...
/-! # GkatK6R01: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def sol2 : Exp Act Tst := (.seq (.ite (BExp.not bT) pA (.seq pA (.ite bT (.test bT) pA))) sol4)
...
theorem ringE4 : EquivBA sol4 (.ite (BExp.not bT) (.seq pA sol2) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite (BExp.not bT) pA (.seq pA (.ite bT (.test bT) pA))) sol4)) (EquivBA.base (Equiv.refl _)))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol4) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 (BExp.not bT) pA (.seq pA (.ite bT (.test bT) pA)) sol4)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.base (Equiv.refl _)) (EquivBA.base (Equiv.s1 pA (.ite bT (.test bT) pA) sol4)))

...
/-! # GkatK6R03: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def sol2 : Exp Act Tst := (.seq (.wh bT (.seq pA (.ite (BExp.not bT) pA (.test BExp.zero)))) (.seq pA sol4))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol4) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.base (salomaa_solution_exists bT (.seq pA (.ite (BExp.not bT) pA (.test BExp.zero))) (.seq pA sol4)))

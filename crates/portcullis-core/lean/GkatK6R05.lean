...
/-! # GkatK6R05: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [0, 1, 2] header=2 exit=bT; the walk plan and every proof
...
def sol0 : Exp Act Tst := (.seq (.wh (BExp.not bT) pA) (.seq pA sol1))

private theorem habs2 : EquivBA (.seq (.test bT) sol2) (.test bT) :=
  test_header_absorb bT (BExp.not bT) BODY (himp_intro_dneg bT)

theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol0) (.test bT)) :=
...
theorem ringE0 : EquivBA sol0 (.ite (BExp.not bT) (.seq pA sol0) (.ite bT (.seq pA sol1) (.test BExp.zero))) :=

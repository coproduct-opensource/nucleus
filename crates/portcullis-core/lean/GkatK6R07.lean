...
/-! # GkatK6R07: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.test (BExp.not bT))) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA))))))
def fP : Exp Act Tst := (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.seq (Exp.wh bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test BExp.zero) pA))) pA)) (Exp.test bT))))
...
def BODY : Exp Act Tst := (.seq pA (.seq (.wh bT (.seq pA (.ite (BExp.not bT) pA (.test BExp.zero)))) (.seq pA (.ite bT (.test bT) pA))))
def sol1 : Exp Act Tst := (.seq (.wh (BExp.not bT) BODY) (.test bT))
def sol0 : Exp Act Tst := (.seq (.ite bT (.test bT) pA) sol1)
def sol2 : Exp Act Tst := (.seq (.wh bT (.seq pA (.ite (BExp.not bT) pA (.test BExp.zero)))) (.seq pA sol0))
def sol3 : Exp Act Tst := (.seq (.ite (BExp.not bT) pA (.test BExp.zero)) sol2)
...
theorem ringE1 : EquivBA sol1 (.ite (BExp.not bT) (.seq pA sol2) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh bT (.seq pA (.ite (BExp.not bT) pA (.test BExp.zero)))) (.seq pA (.ite bT (.test bT) pA))) sol1))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.trans (EquivBA.base (Equiv.s1 (.wh bT (.seq pA (.ite (BExp.not bT) pA (.test BExp.zero)))) (.seq pA (.ite bT (.test bT) pA)) sol1))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol0) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.base (salomaa_solution_exists bT (.seq pA (.ite (BExp.not bT) pA (.test BExp.zero))) (.seq pA sol0)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite (BExp.not bT) pA (.test BExp.zero)) sol2)) (EquivBA.base (Equiv.refl _)))
    (EquivBA.trans (EquivBA.base (Equiv.u2 bT (.seq pA sol3) (.seq pA sol0)))
    (else_expand (BExp.not bT) bT (himp_dneg bT)))))

theorem ringE3 : EquivBA sol3 (.ite (BExp.not bT) (.seq pA sol2) (.test BExp.zero)) :=
  EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 (BExp.not bT) pA (.test BExp.zero) sol2)))
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl (Sum.inr ())))))) => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl (Sum.inr ())))))))) a = bval W (QAut.hlt 0) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl (Sum.inr ())))))) => exact (List.Mem.head _)

...
/-! # GkatK6R08: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.ite bT (Exp.seq pA (Exp.test bT)) (Exp.test (BExp.not bT))))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.ite bT (Exp.seq pA (Exp.test bT)) (Exp.test (BExp.not bT))))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [2, 3, 4] header=3 exit=(BExp.not bT); the walk plan and every proof
...
def sol2 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.test (BExp.not bT)) pA) sol3)
...
theorem ringE3 : EquivBA sol3 (.ite bT (.seq pA sol4) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.ite bT pA (.test BExp.zero)) (.ite (BExp.not bT) (.test (BExp.not bT)) pA)) sol3))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.base (Equiv.s1 (.ite bT pA (.test BExp.zero)) (.ite (BExp.not bT) (.test (BExp.not bT)) pA) sol3)))) (EquivBA.base (Equiv.refl _)))

theorem ringE2 : EquivBA sol2 (.ite bT (.seq pA sol3) (.test (BExp.not bT))) :=
  (EquivBA.trans (park_solves (BExp.not bT) pA sol3 habs3)
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => 4
  | .inr none => 0
  ...
  | .inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => 4
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
    ...
    | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 4, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))))))), rfl⟩

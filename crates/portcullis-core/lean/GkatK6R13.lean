...
/-! # GkatK6R13: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.test (BExp.not bT))) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.test (BExp.not bT))) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)))))
...
theorem ringE4 : EquivBA sol4 (.ite (BExp.not bT) (.seq pA sol2) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.ite (BExp.not bT) pA (.test BExp.zero)) (.ite bT (.test bT) pA)) sol4))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.base (Equiv.s1 (.ite (BExp.not bT) pA (.test BExp.zero)) (.ite bT (.test bT) pA) sol4)))) (EquivBA.base (Equiv.refl _)))

...
/-! # GkatK6R14: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite (BExp.not bT) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.test bT)) (Exp.wh bT (Exp.seq (Exp.seq pA pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) pA) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq pA pA) (Exp.test (BExp.not bT))))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [1, 2, 3] header=2 exit=(BExp.not bT); the walk plan and every proof
...
def sol3 : Exp Act Tst := (.seq pA sol1)
def sol0 : Exp Act Tst := (.ite (BExp.not bT) (.seq pA sol1) (.ite bT (.seq pA sol3) (.test BExp.zero)))
...
theorem ringE3 : EquivBA sol3 (.ite (BExp.not bT) (.seq pA sol1) (.ite bT (.seq pA sol1) (.test BExp.zero))) :=
  both_arms (BExp.not bT) bT (.seq pA sol1) (himp_dneg bT)
...
def QAut : GAut Nat Act Tst where
  ...
  hlt
    ...
    | _ => BExp.zero
  trans
    | 0 => [((BExp.not bT), (), 1), (bT, (), 3)]
    | 1 => [(bT, (), 2)]
    | 2 => [(bT, (), 3)]
    | 3 => [((BExp.not bT), (), 1), (bT, (), 1)]
    | _ => []
  start := 0
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inl (Sum.inl ())))) => 3
  | .inl (some (Sum.inr (Sum.inl (Sum.inr ())))) => 1
  ...
  | .inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))) => 1
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl none =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨3, qstep_0_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
  ...
  | Sum.inl (some (Sum.inr (Sum.inl (Sum.inl ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl ())))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inl (some (Sum.inr (Sum.inl (Sum.inr ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr ())))))) a = bval W (QAut.hlt 1) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))))) a = bval W (QAut.hlt 1) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inl (Sum.inl ())))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inl (some (Sum.inr (Sum.inl (Sum.inr ())))) => exact (List.Mem.tail _ (List.Mem.head _))
    ...
    | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))) => exact (List.Mem.tail _ (List.Mem.head _))
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 3, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inl (Sum.inl ()))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inl (Sum.inl ()))))))))), rfl⟩

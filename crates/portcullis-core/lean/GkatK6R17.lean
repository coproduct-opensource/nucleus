...
/-! # GkatK6R17: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite (BExp.not bT) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.test bT)) (Exp.wh bT (Exp.seq (Exp.wh bT pA) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.wh bT pA) (Exp.ite bT (Exp.test BExp.zero) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.wh bT pA) (Exp.ite bT (Exp.test BExp.zero) pA)) (Exp.test (BExp.not bT))))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [0, 1, 2] header=2 exit=(BExp.not bT); the walk plan and every proof
...
def sol0 : Exp Act Tst := (.seq (.wh bT pA) (.seq pA sol1))
...
theorem ringE2 : EquivBA sol2 (.ite bT (.seq pA sol0) (.test (BExp.not bT))) :=
...
theorem ringE0 : EquivBA sol0 (.ite bT (.seq pA sol0) (.ite (BExp.not bT) (.seq pA sol1) (.test BExp.zero))) :=
  ...
  | Sum.inl none =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨0, qstep_0_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst

...
/-! # GkatK6R19: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.wh bT pA)))))
def fP : Exp Act Tst := (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.seq (Exp.wh bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA))) pA)) (Exp.test bT))))
...
def BODY : Exp Act Tst := (.seq pA (.seq (.wh bT (.seq pA (.seq (.wh bT pA) pA))) (.seq pA (.ite bT (.test bT) pA))))
def sol1 : Exp Act Tst := (.seq (.wh (BExp.not bT) BODY) (.test bT))
def sol0 : Exp Act Tst := (.seq (.ite bT (.test bT) pA) sol1)
def sol2 : Exp Act Tst := (.seq (.wh bT (.seq pA (.seq (.wh bT pA) pA))) (.seq pA sol0))
def sol3 : Exp Act Tst := (.seq (.wh bT pA) (.seq pA sol2))
...
theorem ringE1 : EquivBA sol1 (.ite (BExp.not bT) (.seq pA sol2) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh bT (.seq pA (.seq (.wh bT pA) pA))) (.seq pA (.ite bT (.test bT) pA))) sol1))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.trans (EquivBA.base (Equiv.s1 (.wh bT (.seq pA (.seq (.wh bT pA) pA))) (.seq pA (.ite bT (.test bT) pA)) sol1))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol0) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.base (salomaa_solution_exists bT (.seq pA (.seq (.wh bT pA) pA)) (.seq pA sol0)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh bT pA) pA) sol2))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.base (Equiv.s1 (.wh bT pA) pA sol2)))) (EquivBA.base (Equiv.refl _)))
    (EquivBA.trans (EquivBA.base (Equiv.u2 bT (.seq pA sol3) (.seq pA sol0)))
    (else_expand (BExp.not bT) bT (himp_dneg bT)))))

theorem ringE3 : EquivBA sol3 (.ite bT (.seq pA sol3) (.ite (BExp.not bT) (.seq pA sol2) (.test BExp.zero))) :=
...
def QAut : GAut Nat Act Tst where
  ...
  trans
    | 0 => [((BExp.not bT), (), 1)]
    | 1 => [((BExp.not bT), (), 2)]
    | 2 => [((BExp.not bT), (), 0), (bT, (), 3)]
    | 3 => [(bT, (), 3), ((BExp.not bT), (), 2)]
    | _ => []
  start := 0
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))) => 3
  ...
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inl ())))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inl ()))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨3, qstep_2_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))) ()
        ...
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))

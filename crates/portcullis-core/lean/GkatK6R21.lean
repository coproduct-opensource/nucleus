...
/-! # GkatK6R21: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) pA) (Exp.seq (Exp.wh bT pA) (Exp.ite bT (Exp.test BExp.zero) pA))) (Exp.test (BExp.not bT)))))
def fP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.seq (Exp.wh bT pA) (Exp.ite bT (Exp.test BExp.zero) pA))) (Exp.test (BExp.not bT)))))
...
def sol3 : Exp Act Tst := (.seq (.wh bT pA) (.seq pA sol0))
...
theorem ringE3 : EquivBA sol3 (.ite bT (.seq pA sol3) (.ite (BExp.not bT) (.seq pA sol0) (.test BExp.zero))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT pA (.seq pA sol0)))
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl none => 0
  | .inl (some (Sum.inl ())) => 1
  | .inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ())))))) => 2
  | .inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))) => 3
  | .inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))) => 3
  | .inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))) => 0
  | .inr none => 0
  | .inr (some (Sum.inl ())) => 1
  | .inr (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ())))))) => 2
  | .inr (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr (Sum.inr ())))))) => 3
  | .inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))) => 3
  | .inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))) => 0
  | _ => 0
  ...
  | Sum.inl (some (Sum.inl ())) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inl ()))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ())))))))) a = bval W (QAut.hlt 2) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))))) ()
        ...
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
  ...
  | Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨0, qstep_3_false W a hb, rfl⟩
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
        | true =>
            rw [hb] at hst
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ())))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ())))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
        ...
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ())))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))))) a = bval W (QAut.hlt 0) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))) ()
                = some ((), (Sum.inl (some (Sum.inl ())))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
  ...
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ())))))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨3, qstep_2_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ())))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr (Sum.inr ())))))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr (Sum.inr ()))))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨0, qstep_3_false W a hb, rfl⟩
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
        | true =>
            rw [hb] at hst
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ())))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
        ...
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ())))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))))) a = bval W (QAut.hlt 0) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ()))))))) ()
                = some ((), (Sum.inr (some (Sum.inl ())))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ())))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
    | Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inl (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))) => exact (List.Mem.head _)
    ...
    | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr (Sum.inr ())))))) => exact (List.Mem.head _)
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 2, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inl (Sum.inl (Sum.inl (Sum.inr ()))))))))))), rfl⟩

...
/-! # GkatK6R22: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.seq pA (Exp.test bT))))))
def fP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq pA (Exp.seq (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA))) pA)) (Exp.test (BExp.not bT)))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [0, 1, 2, 3] header=1 exit=(BExp.not bT); the walk plan and every proof
    recipe follow GkatRingSupportProofs and the GkatCertR1-R6 instances. -/

def BODY : Exp Act Tst := (.seq pA (.seq (.wh (BExp.not bT) (.seq pA (.ite bT pA (.test BExp.zero)))) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA))))
def sol1 : Exp Act Tst := (.seq (.wh bT BODY) (.test (BExp.not bT)))
def sol0 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.test (BExp.not bT)) pA) sol1)
def sol2 : Exp Act Tst := (.seq (.wh (BExp.not bT) (.seq pA (.ite bT pA (.test BExp.zero)))) (.seq pA sol0))
def sol3 : Exp Act Tst := (.seq (.ite bT pA (.test BExp.zero)) sol2)
...
theorem ringE1 : EquivBA sol1 (.ite bT (.seq pA sol2) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh (BExp.not bT) (.seq pA (.ite bT pA (.test BExp.zero)))) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA))) sol1))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.trans (EquivBA.base (Equiv.s1 (.wh (BExp.not bT) (.seq pA (.ite bT pA (.test BExp.zero)))) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA)) sol1))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol3) (.ite bT (.seq pA sol0) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) (.seq pA (.ite bT pA (.test BExp.zero))) (.seq pA sol0)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite bT pA (.test BExp.zero)) sol2)) (EquivBA.base (Equiv.refl _)))
    (else_expand (BExp.not bT) bT (himp_dneg bT))))

theorem ringE3 : EquivBA sol3 (.ite bT (.seq pA sol2) (.test BExp.zero)) :=
  EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 bT pA (.test BExp.zero) sol2)))
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) => 0
  | .inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => 1
  | .inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) => 3
  | .inr none => 0
  | .inr (some (Sum.inl ())) => 1
  | .inr (some (Sum.inr (Sum.inl (Sum.inl ())))) => 2
  | .inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))) => 3
  | .inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))) => 2
  | .inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))) => 0
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inl ())))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨0, qstep_2_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
        | true =>
            rw [hb] at hst
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))) a = bval W (QAut.hlt 0) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        ...
        | true =>
            ...
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl (Sum.inr ()))))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨1, qstep_0_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) a = bval W (QAut.hlt 1) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl ()))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inl ()))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨2, qstep_3_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
  ...
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inl ())))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inl ()))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨3, qstep_2_false W a hb, rfl⟩
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
        | true =>
            rw [hb] at hst
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨2, qstep_3_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))))) a = bval W (QAut.hlt 2) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
        ...
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
  ...
  | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))) a = bval W (QAut.hlt 0) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) => exact (List.Mem.head _)
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => exact (List.Mem.tail _ (List.Mem.head _))
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    ...
    | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
    | Sum.inr (some (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))) => exact (List.Mem.head _)
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 3, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))))))), rfl⟩

...
/-! # GkatK6R23: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.wh (BExp.not bT) pA)))))
def fP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq pA (Exp.seq (Exp.wh (BExp.not bT) (Exp.seq (Exp.wh (BExp.not bT) pA) (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA))) pA)) (Exp.test (BExp.not bT)))))
...
def BODY : Exp Act Tst := (.seq pA (.seq (.wh (BExp.not bT) (.seq pA (.seq (.wh (BExp.not bT) pA) pA))) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA))))
def sol1 : Exp Act Tst := (.seq (.wh bT BODY) (.test (BExp.not bT)))
def sol0 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.test (BExp.not bT)) pA) sol1)
def sol2 : Exp Act Tst := (.seq (.wh (BExp.not bT) (.seq pA (.seq (.wh (BExp.not bT) pA) pA))) (.seq pA sol0))
def sol3 : Exp Act Tst := (.seq (.wh (BExp.not bT) pA) (.seq pA sol2))
...
theorem ringE1 : EquivBA sol1 (.ite bT (.seq pA sol2) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh (BExp.not bT) (.seq pA (.seq (.wh (BExp.not bT) pA) pA))) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA))) sol1))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.trans (EquivBA.base (Equiv.s1 (.wh (BExp.not bT) (.seq pA (.seq (.wh (BExp.not bT) pA) pA))) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA)) sol1))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol3) (.ite bT (.seq pA sol0) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) (.seq pA (.seq (.wh (BExp.not bT) pA) pA)) (.seq pA sol0)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh (BExp.not bT) pA) pA) sol2))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.base (Equiv.s1 (.wh (BExp.not bT) pA) pA sol2)))) (EquivBA.base (Equiv.refl _)))
    (else_expand (BExp.not bT) bT (himp_dneg bT))))

theorem ringE3 : EquivBA sol3 (.ite (BExp.not bT) (.seq pA sol3) (.ite bT (.seq pA sol2) (.test BExp.zero))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) pA (.seq pA sol2)))
    (else_expand (BExp.not bT) bT (himp_dneg bT))
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inr (Sum.inr ())))) => 3
  | .inr none => 0
  | .inr (some (Sum.inl ())) => 1
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inr ())))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr ())))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
  ...
  onto_states := by
    ...
    | 3, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inr ()))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inr ()))))))))), rfl⟩

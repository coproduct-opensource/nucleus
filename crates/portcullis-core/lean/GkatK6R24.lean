...
/-! # GkatK6R24: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.test (BExp.not bT))) (Exp.test bT))) (Exp.test (BExp.not bT))))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.test (BExp.not bT))) (Exp.test bT))) (Exp.test (BExp.not bT))))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [2, 3] header=2 exit=(BExp.not bT); the walk plan and every proof
...
def sol3 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.seq pA sol4) pA) sol2)
...
private theorem abs4 : EquivBA (.seq sol4 sol2) sol4 :=
  habs2
...
theorem ringE2 : EquivBA sol2 (.ite bT (.seq pA sol3) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite (BExp.not bT) (.seq pA sol4) pA) sol2)) (EquivBA.base (Equiv.refl _)))

theorem ringE3 : EquivBA sol3 (.ite (BExp.not bT) (.seq pA sol4) (.ite bT (.seq pA sol2) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 (BExp.not bT) (.seq pA sol4) pA sol2)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA sol4 sol2))
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
        | true =>
            rw [hb] at hst
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
      · rw [autStep_unit] at hst
  ...
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
    ...
    | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 4, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))))))), rfl⟩

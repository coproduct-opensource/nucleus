...
/-! # GkatK6R25: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.seq pA (Exp.test bT))))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.seq pA (Exp.test bT))))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [2, 3] header=3 exit=bT; the walk plan and every proof
    recipe follow GkatRingSupportProofs and the GkatCertR1-R6 instances. -/

def sol4 : Exp Act Tst := (.test bT)
def BODY : Exp Act Tst := (.seq pA (.ite bT (.seq pA sol4) pA))
def sol3 : Exp Act Tst := (.seq (.wh (BExp.not bT) BODY) (.test bT))
...
private theorem habs3 : EquivBA (.seq (.test bT) sol3) (.test bT) :=
  test_header_absorb bT (BExp.not bT) BODY (himp_intro_dneg bT)

private theorem abs4 : EquivBA (.seq sol4 sol3) sol4 :=
  habs3

theorem ringE3 : EquivBA sol3 (.ite (BExp.not bT) (.seq pA sol2) (.test bT)) :=

...
/-! # GkatK6R27: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.ite bT (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq pA pA) (Exp.wh bT pA)) pA))) (Exp.test BExp.zero))
def fP : Exp Act Tst := (Exp.ite bT (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.wh bT pA)) pA))) (Exp.test BExp.zero))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [1, 4] header=4 exit=(BExp.not bT); the walk plan and every proof
    recipe follow GkatRingSupportProofs and the GkatCertR1-R6 instances. -/

def L3 : LevelG Act Tst := ([(bT, pA)], [], (Exp.test (BExp.not bT)))

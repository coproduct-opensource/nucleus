...
/-! # GkatK6R28: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq pA pA) (Exp.wh (BExp.not bT) pA)) pA)))
def fP : Exp Act Tst := (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.wh (BExp.not bT) pA)) pA)))

...
/-! # GkatK6R29: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.ite (BExp.not bT) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.seq pA (Exp.test bT))))) (Exp.seq pA (Exp.test bT)))
def fP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.ite bT (Exp.seq pA (Exp.test bT)) (Exp.test (BExp.not bT)))) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.ite bT (Exp.seq pA (Exp.test bT)) (Exp.test (BExp.not bT)))) (Exp.test bT)))))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol1) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite bT (.seq pA sol3) pA) sol2)) (EquivBA.base (Equiv.refl _)))

theorem ringE1 : EquivBA sol1 (.ite (BExp.not bT) (.seq pA sol2) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 bT (.seq pA sol3) pA sol2)))
    ...
    (EquivBA.trans (EquivBA.base (Equiv.u2 bT (.seq pA sol3) (.seq pA sol2)))
  ...
  | Sum.inl (some (Sum.inl (Sum.inl ()))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨3, qstep_1_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst

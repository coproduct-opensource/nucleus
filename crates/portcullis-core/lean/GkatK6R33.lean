...
/-! # GkatK6R33: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.test (BExp.not bT))) (Exp.test bT)))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.test (BExp.not bT))) (Exp.test bT)))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [2, 3, 4] header=3 exit=bT; the walk plan and every proof
...
def sol2 : Exp Act Tst := (.seq (.ite bT (.test bT) pA) sol3)
...
def sol4 : Exp Act Tst := (.seq (.ite (BExp.not bT) pA (.test BExp.zero)) sol2)
...
theorem ringE3 : EquivBA sol3 (.ite (BExp.not bT) (.seq pA sol4) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.ite (BExp.not bT) pA (.test BExp.zero)) (.ite bT (.test bT) pA)) sol3))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.base (Equiv.s1 (.ite (BExp.not bT) pA (.test BExp.zero)) (.ite bT (.test bT) pA) sol3)))) (EquivBA.base (Equiv.refl _)))

theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol3) (.test bT)) :=
  (park_solves bT pA sol3 habs3)

theorem ringE4 : EquivBA sol4 (.ite (BExp.not bT) (.seq pA sol2) (.test BExp.zero)) :=

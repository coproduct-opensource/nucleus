...
/-! # GkatK6R34: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.ite bT (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.seq pA pA) (Exp.wh bT pA)) pA))) (Exp.test BExp.zero))
def fP : Exp Act Tst := (Exp.ite bT (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.wh bT pA)) pA))) (Exp.test BExp.zero))
...
def sol1 : Exp Act Tst := (.seq (.ite bT (.seq pA sol2) pA) sol4)
...
theorem ringE4 : EquivBA sol4 (.ite bT (.seq pA sol1) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite bT (.seq pA sol2) pA) sol4)) (EquivBA.base (Equiv.refl _)))

theorem ringE1 : EquivBA sol1 (.ite (BExp.not bT) (.seq pA sol4) (.ite bT (.seq pA sol2) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 bT (.seq pA sol2) pA sol4)))
    ...
    (EquivBA.trans (EquivBA.base (Equiv.u2 bT (.seq pA sol2) (.seq pA sol4)))
  ...
  | Sum.inl (some (Sum.inl (Sum.inl ()))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨4, qstep_1_false W a hb, rfl⟩
        | true =>
            rw [hb] at hst

...
/-! # GkatK6R41: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.wh bT pA) (Exp.ite bT (Exp.test BExp.zero) pA)) (Exp.test (BExp.not bT))))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.wh bT pA) (Exp.ite bT (Exp.test BExp.zero) pA)) (Exp.test (BExp.not bT))))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [2, 3, 4] header=2 exit=(BExp.not bT); the walk plan and every proof
    recipe follow GkatRingSupportProofs and the GkatCertR1-R6 instances. -/

def BODY : Exp Act Tst := (.seq pA (.seq (.wh bT pA) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA))))
...
def sol4 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.test (BExp.not bT)) pA) sol2)
...
theorem ringE2 : EquivBA sol2 (.ite bT (.seq pA sol3) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh bT pA) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA))) sol2))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.trans (EquivBA.base (Equiv.s1 (.wh bT pA) (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA)) sol2))
...
theorem ringE4 : EquivBA sol4 (.ite bT (.seq pA sol2) (.test (BExp.not bT))) :=
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => 3
  | .inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => 4
  | .inr none => 0
  | .inr (some (Sum.inl (Sum.inl ()))) => 1
  | .inr (some (Sum.inl (Sum.inr ()))) => 1
  | .inr (some (Sum.inr (Sum.inl ()))) => 2
  | .inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => 3
  | .inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => 4
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inl ()))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inl ())))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))) ()
        ...
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))) ()
                = some ((), (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
    ...
    | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 3, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inl (Sum.inl ())))))))))), rfl⟩
    | 4, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inl (Sum.inr (Sum.inr ()))))))))))), rfl⟩

...
/-! # GkatK6R43: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.ite (BExp.not bT) (Exp.wh (BExp.not bT) (Exp.seq (Exp.seq (Exp.seq pA pA) (Exp.wh (BExp.not bT) pA)) (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)))) (Exp.test BExp.zero))
def fP : Exp Act Tst := (Exp.ite (BExp.not bT) (Exp.wh (BExp.not bT) (Exp.seq (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.wh (BExp.not bT) pA)) (Exp.ite bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.test (BExp.not bT))))) (Exp.test BExp.zero))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [1, 2, 3, 4] header=4 exit=bT; the walk plan and every proof
...
theorem ringE4 : EquivBA sol4 (.ite (BExp.not bT) (.seq pA sol1) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq pA (.seq (.wh (BExp.not bT) pA) (.seq pA (.ite bT (.test bT) pA)))) sol4))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh (BExp.not bT) pA) (.seq pA (.ite bT (.test bT) pA))) sol4))
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) => 3
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))

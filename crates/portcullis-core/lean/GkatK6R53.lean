...
/-! # GkatK6R53: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.seq pA (Exp.test bT))))))
def fP : Exp Act Tst := (Exp.ite bT (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.seq pA (Exp.seq (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA))) pA)) (Exp.test (BExp.not bT))))) (Exp.test BExp.zero))
...
def sol3 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.test (BExp.not bT)) pA) sol1)
def sol2 : Exp Act Tst := (.seq (.wh (BExp.not bT) (.seq pA (.ite bT pA (.test BExp.zero)))) (.seq pA sol3))
def sol4 : Exp Act Tst := (.seq (.ite bT pA (.test BExp.zero)) sol2)
...
theorem ringE3 : EquivBA sol3 (.ite bT (.seq pA sol1) (.test (BExp.not bT))) :=
  (EquivBA.trans (park_solves (BExp.not bT) pA sol1 habs1)
    (EquivBA.ite_guard (dneg_bval bT)))

theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol4) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) (.seq pA (.ite bT pA (.test BExp.zero))) (.seq pA sol3)))
...
theorem ringE4 : EquivBA sol4 (.ite bT (.seq pA sol2) (.test BExp.zero)) :=
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) => 4
  | .inr none => 0
  | .inr (some (Sum.inl (Sum.inl ()))) => 1
  | .inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))) => 2
  | .inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) => 4
  | .inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) => 2
  | .inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => 3
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨2, qstep_4_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
  ...
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))) ()
                = some ((), (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨4, qstep_2_false W a hb, rfl⟩
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
        | true =>
            rw [hb] at hst
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ())))))))) ()
  ...
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
    ...
    | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl ()))))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
    ...
    | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inr ())))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 4, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))))))), rfl⟩

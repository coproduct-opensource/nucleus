...
/-! # GkatK6R60: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA)) (Exp.test bT)))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA)) (Exp.test bT)))))
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol3) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.ite bT pA (.test BExp.zero)) (.ite bT (.test bT) pA)) sol2))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.base (Equiv.s1 (.ite bT pA (.test BExp.zero)) (.ite bT (.test bT) pA) sol2)))) (EquivBA.base (Equiv.refl _)))

...
/-! # GkatK6R61: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite bT (Exp.test bT) pA) (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.wh (BExp.not bT) pA) (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA)) (Exp.test bT)))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh bT pA)) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.wh (BExp.not bT) pA) (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA)) (Exp.test bT)))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [2, 3, 4] header=2 exit=bT; the walk plan and every proof
    recipe follow GkatRingSupportProofs and the GkatCertR1-R6 instances. -/

def BODY : Exp Act Tst := (.seq pA (.seq (.wh (BExp.not bT) pA) (.seq pA (.ite bT (.test bT) pA))))
...
def sol4 : Exp Act Tst := (.seq (.ite bT (.test bT) pA) sol2)
...
theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol3) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq (.wh (BExp.not bT) pA) (.seq pA (.ite bT (.test bT) pA))) sol2))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) (EquivBA.trans (EquivBA.base (Equiv.s1 (.wh (BExp.not bT) pA) (.seq pA (.ite bT (.test bT) pA)) sol2))

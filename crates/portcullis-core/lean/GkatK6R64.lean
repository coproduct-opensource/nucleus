...
/-! # GkatK6R64: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.ite (BExp.not bT) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq pA pA) (Exp.wh (BExp.not bT) pA)) pA))) (Exp.test BExp.zero))
def fP : Exp Act Tst := (Exp.ite (BExp.not bT) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA)) (Exp.wh (BExp.not bT) pA)) pA))) (Exp.test BExp.zero))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [1, 4] header=4 exit=bT; the walk plan and every proof
...
def BODY : Exp Act Tst := (.seq pA (.ite (BExp.not bT) (.seq pA sol2) pA))
def sol4 : Exp Act Tst := (.seq (.wh (BExp.not bT) BODY) (.test bT))
def sol1 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.seq pA sol2) pA) sol4)
...
private theorem absF3 : EquivBA (.seq (guardedFold L3.2.1 L3.2.2) sol4) (guardedFold L3.2.1 L3.2.2) :=
  habs4
private theorem abs3 : EquivBA (.seq sol3 sol4) sol3 :=
  EquivBA.trans (EquivBA.base (Equiv.s1 (.wh (orGuards L3.1) (bodyFold L3.1)) (guardedFold L3.2.1 L3.2.2) sol4))
    (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) absF3)

private theorem abs2 : EquivBA (.seq sol2 sol4) sol2 :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 (BExp.not bT) (.seq pA sol3) (.ite bT (.seq pA sol3) (.test BExp.zero)) sol4)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA sol3 sol4))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) abs3)) (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 bT (.seq pA sol3) (.test BExp.zero) sol4)))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA sol3 sol4))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) abs3)) (EquivBA.base (Equiv.s2 sol4))))))
...
theorem ringE4 : EquivBA sol4 (.ite (BExp.not bT) (.seq pA sol1) (.test bT)) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists (BExp.not bT) BODY (.test bT)))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite (BExp.not bT) (.seq pA sol2) pA) sol4)) (EquivBA.base (Equiv.refl _)))

theorem ringE1 : EquivBA sol1 (.ite (BExp.not bT) (.seq pA sol2) (.ite bT (.seq pA sol4) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 (BExp.not bT) (.seq pA sol2) pA sol4)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA sol2 sol4))
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inl (Sum.inr (Sum.inr ())))) => 4
  | .inr none => 0
  ...
  | .inr (some (Sum.inl (Sum.inr (Sum.inr ())))) => 4
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inl (Sum.inl ()))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨4, qstep_1_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
  ...
  | Sum.inl (some (Sum.inl (Sum.inr (Sum.inr ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inl (Sum.inr (Sum.inr ())))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inr ())))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr (Sum.inr ())))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inl (Sum.inr (Sum.inr ())))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
    ...
    | Sum.inr (some (Sum.inl (Sum.inr (Sum.inr ())))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 4, _ => exact ⟨(Sum.inl (some (Sum.inl (Sum.inr (Sum.inr ()))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inl (Sum.inr (Sum.inr ()))))))))), rfl⟩

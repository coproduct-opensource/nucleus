...
/-! # GkatK6R65: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.seq pA (Exp.test (BExp.not bT)))))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.seq pA (Exp.test (BExp.not bT)))))))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [2, 3] header=3 exit=(BExp.not bT); the walk plan and every proof
    recipe follow GkatRingSupportProofs and the GkatCertR1-R6 instances. -/

def sol4 : Exp Act Tst := (.test (BExp.not bT))
def BODY : Exp Act Tst := (.seq pA (.ite bT (.seq pA sol4) pA))
def sol3 : Exp Act Tst := (.seq (.wh bT BODY) (.test (BExp.not bT)))
def sol2 : Exp Act Tst := (.seq (.ite bT (.seq pA sol4) pA) sol3)
def L1 : LevelG Act Tst := ([((BExp.not bT), pA)], [(bT, Exp.seq pA sol2)], (Exp.test BExp.zero))
def sol1 : Exp Act Tst := levelSol L1
def sol0 : Exp Act Tst := (.ite (BExp.not bT) (.seq pA sol1) (.ite bT (.seq pA sol1) (.test BExp.zero)))

private theorem habs3 : EquivBA (.seq (.test (BExp.not bT)) sol3) (.test (BExp.not bT)) :=
  test_header_absorb (BExp.not bT) bT BODY (himp_self (BExp.not bT))

private theorem abs4 : EquivBA (.seq sol4 sol3) sol4 :=
  habs3

theorem ringE3 : EquivBA sol3 (.ite bT (.seq pA sol2) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite bT (.seq pA sol4) pA) sol3)) (EquivBA.base (Equiv.refl _)))

theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol3) (.ite bT (.seq pA sol4) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 bT (.seq pA sol4) pA sol3)))
    (EquivBA.trans (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA sol4 sol3))
      (EquivBA.seq_c (EquivBA.base (Equiv.refl _)) abs4)) (EquivBA.base (Equiv.refl _)))
    (EquivBA.trans (EquivBA.base (Equiv.u2 bT (.seq pA sol4) (.seq pA sol3)))
...
def QAut : GAut Nat Act Tst where
  ...
  trans
    ...
    | 1 => [((BExp.not bT), (), 1), (bT, (), 2)]
    | 2 => [((BExp.not bT), (), 3), (bT, (), 4)]
    | 3 => [(bT, (), 2)]
    | 4 => []
    | _ => []
  start := 0
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))) => 3
  ...
  | .inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) => 4
  | _ => 0
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inl ()))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨4, qstep_2_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))))) a = bval W (QAut.hlt 3) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
  ...
  | Sum.inr (some (Sum.inr (Sum.inl ()))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inl ())))) ()
                = some ((), (Sum.inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))))) := by rfl
            rw [hred] at hst
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨4, qstep_2_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            refine ⟨(Sum.inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))), ?_, rfl⟩
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            rw [autStep_unit, hb]
            rfl
  | Sum.inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => false) SUM (Sum.inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
        | true =>
            rw [hb] at hst
            have hred : autStep (fun _ (_ : Unit) => true) SUM (Sum.inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ())))))) ()
                = none := by rfl
            rw [hred] at hst
            exact absurd hst (by simp)
      · rw [autStep_unit] at hst
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))))
    ...
    | Sum.inr (some (Sum.inr (Sum.inr (Sum.inr (Sum.inl ()))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))
  onto_states := by
    intro q hq
    match q, hq with
    ...
    | 3, _ => exact ⟨(Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inr ())))))), (List.mem_append.mpr (Or.inl (List.mem_map_of_mem (List.Mem.tail _ (List.mem_map_of_mem (GkatTotalization.thompson_states_complete eP (Sum.inr (Sum.inr (Sum.inl (Sum.inr ())))))))))), rfl⟩
...
theorem qsol_solves : SolvesBA QAut qsol := by
  ...
  | 2, _ => exact ringE2
  | 3, _ => exact ringE3
  | 4, _ => exact EquivBA.base (Equiv.refl _)

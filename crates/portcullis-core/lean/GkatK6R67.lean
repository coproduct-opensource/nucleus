...
/-! # GkatK6R67: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.wh bT pA)))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.wh bT pA)))))
...
private theorem absF4 : EquivBA (.seq (guardedFold L4.2.1 L4.2.2) sol3) (guardedFold L4.2.1 L4.2.2) :=
  habs3
private theorem abs4 : EquivBA (.seq sol4 sol3) sol4 :=
  EquivBA.trans (EquivBA.base (Equiv.s1 (.wh (orGuards L4.1) (bodyFold L4.1)) (guardedFold L4.2.1 L4.2.2) sol3))

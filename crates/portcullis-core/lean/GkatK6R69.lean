...
/-! # GkatK6R69: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.wh bT pA)))))
def fP : Exp Act Tst := (Exp.ite (BExp.not bT) (Exp.wh (BExp.not bT) (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq pA (Exp.seq (Exp.wh bT (Exp.seq (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA))) pA)) (Exp.test bT)))) (Exp.test BExp.zero))
...
def sol3 : Exp Act Tst := (.seq (.ite bT (.test bT) pA) sol1)
def sol2 : Exp Act Tst := (.seq (.wh bT (.seq pA (.seq (.wh bT pA) pA))) (.seq pA sol3))
def sol4 : Exp Act Tst := (.seq (.wh bT pA) (.seq pA sol2))
...
theorem ringE3 : EquivBA sol3 (.ite (BExp.not bT) (.seq pA sol1) (.test bT)) :=
  (park_solves bT pA sol1 habs1)

theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol3) (.ite bT (.seq pA sol4) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.base (salomaa_solution_exists bT (.seq pA (.seq (.wh bT pA) pA)) (.seq pA sol3)))
...
theorem ringE4 : EquivBA sol4 (.ite bT (.seq pA sol4) (.ite (BExp.not bT) (.seq pA sol2) (.test BExp.zero))) :=
...
def qmap : Sum (Option (certifiedThompson Act Tst eP).State)
  ...
  | .inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))) => 4
...
theorem qmap_bisim : GAutBisim W SUM QAut (fun s q => qmap s = q) := by
  ...
  | Sum.inl (some (Sum.inr (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | false =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨1, qstep_3_false W a hb, rfl⟩
        | true =>
            rw [hb] at hst
  ...
  | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))) =>
      first | simp only [qmap] | skip
      refine ⟨fun a => ?_, fun a q s' hst => ?_, fun a q s2' hst => ?_⟩
      · show bval W (SUM.hlt (Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))))) a = bval W (QAut.hlt 4) a
        rw [bval_hlt_unit SUM, bval_hlt_unit QAut]
        cases hb : W () a <;> rfl
      · rw [autStep_unit] at hst
        cases hb : W () a with
...
def qquot : UniformBehavioralGAutQuotient SUM QAut where
  ...
  maps_states := by
    ...
    | Sum.inr (some (Sum.inl (Sum.inr (Sum.inl (Sum.inr (Sum.inl (Sum.inl (Sum.inr ())))))))) => exact (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))))

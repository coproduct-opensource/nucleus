...
/-! # GkatK6R74: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq (Exp.seq pA pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq (Exp.seq pA pA) (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA))))
...
theorem ringE4 : EquivBA sol4 (.ite bT (.seq pA sol2) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.trans (EquivBA.base (Equiv.s1 pA (.seq pA (.ite (BExp.not bT) (.test (BExp.not bT)) pA)) sol4))

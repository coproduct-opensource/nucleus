...
/-! # GkatK6R76: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.seq (Exp.seq (Exp.ite (BExp.not bT) (Exp.test (BExp.not bT)) pA) (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.seq pA (Exp.test (BExp.not bT)))))))
def fP : Exp Act Tst := (Exp.seq (Exp.seq pA (Exp.wh (BExp.not bT) pA)) (Exp.wh bT (Exp.seq pA (Exp.ite bT (Exp.ite (BExp.not bT) (Exp.test BExp.zero) pA) (Exp.seq pA (Exp.test (BExp.not bT)))))))
...
def BODY : Exp Act Tst := (.seq pA (.ite (BExp.not bT) (.seq pA sol4) pA))
def sol3 : Exp Act Tst := (.seq (.wh bT BODY) (.test (BExp.not bT)))
def sol2 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.seq pA sol4) pA) sol3)
...
theorem ringE3 : EquivBA sol3 (.ite bT (.seq pA sol2) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite (BExp.not bT) (.seq pA sol4) pA) sol3)) (EquivBA.base (Equiv.refl _)))

theorem ringE2 : EquivBA sol2 (.ite (BExp.not bT) (.seq pA sol4) (.ite bT (.seq pA sol3) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 (BExp.not bT) (.seq pA sol4) pA sol3)))

...
/-! # GkatK6R80: emitted instance certificate (machine-generated; see emit_cert.py) -/
...
def eP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq pA pA) (Exp.wh bT pA)) pA)))
def fP : Exp Act Tst := (Exp.wh bT (Exp.seq pA (Exp.ite (BExp.not bT) (Exp.seq (Exp.seq (Exp.ite bT (Exp.test BExp.zero) pA) (Exp.ite bT (Exp.test bT) pA)) (Exp.wh bT pA)) pA)))
...
/-! ## Ring solutions (machine-generated; see emit_ring.py).
    SCC [0, 1] header=0 exit=(BExp.not bT); the walk plan and every proof
...
def sol1 : Exp Act Tst := (.seq (.ite (BExp.not bT) (.seq pA sol2) pA) sol0)
...
theorem ringE0 : EquivBA sol0 (.ite bT (.seq pA sol1) (.test (BExp.not bT))) :=
  EquivBA.trans (EquivBA.base (salomaa_solution_exists bT BODY (.test (BExp.not bT))))
    (EquivBA.ite_c (EquivBA.base (Equiv.s1 pA (.ite (BExp.not bT) (.seq pA sol2) pA) sol0)) (EquivBA.base (Equiv.refl _)))

theorem ringE1 : EquivBA sol1 (.ite (BExp.not bT) (.seq pA sol2) (.ite bT (.seq pA sol0) (.test BExp.zero))) :=
  (EquivBA.trans (EquivBA.symm (EquivBA.base (Equiv.u5 (BExp.not bT) (.seq pA sol2) pA sol0)))
  ...
  | Sum.inl (some (Sum.inl ())) =>
      ...
      · rw [autStep_unit] at hst
        ...
        | true =>
            ...
            have hs := congrArg Prod.snd (Option.some.inj hst)
            subst hs
            exact ⟨0, qstep_1_true W a hb, rfl⟩
      · rw [autStep_unit] at hst
        cases hb : W () a with
        | false =>
            rw [hb] at hst

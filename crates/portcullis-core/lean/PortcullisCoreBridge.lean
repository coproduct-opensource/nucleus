...
Proves two things about the Aeneas-generated `nucleus_ifc_kernel.CapabilityLevel`:
...
theorem meet_never_fails (a b : CapabilityLevel) :
    ∃ r, nucleus_ifc_kernel.CapabilityLevel.meet a b = .ok r := by
...
theorem lattice_meet_eq_inf (a b : CapabilityLattice) :
    nucleus_ifc_kernel.CapabilityLattice.meet a b = .ok (a ⊓ b) := by
  show _ = Result.ok (latticeInf a b)
  simp [nucleus_ifc_kernel.CapabilityLattice.meet, meet_eq_inf, latticeInf]

open ProductLattice in
/-- The Aeneas-generated `CapabilityLattice.join` computes pointwise sup. -/
theorem lattice_join_eq_sup (a b : CapabilityLattice) :
    nucleus_ifc_kernel.CapabilityLattice.join a b = .ok (a ⊔ b) := by
  show _ = Result.ok (latticeSup a b)
  simp [nucleus_ifc_kernel.CapabilityLattice.join, join_eq_sup, latticeSup]
...
theorem lattice_implies_eq_himp (a b : CapabilityLattice) :
    nucleus_ifc_kernel.CapabilityLattice.implies a b = .ok (a ⇨ b) := by
  show _ = Result.ok (latticeHImp a b)
  simp [nucleus_ifc_kernel.CapabilityLattice.implies, implies_eq_himp, latticeHImp]

open ProductLattice in
/-- The Aeneas-generated `CapabilityLattice.bottom` is the lattice ⊥. -/
theorem lattice_bottom_eq_bot :
    nucleus_ifc_kernel.CapabilityLattice.bottom = .ok ⊥ := by
  show _ = Result.ok latticeBot
  simp [nucleus_ifc_kernel.CapabilityLattice.bottom, latticeBot]
  constructor <;> rfl

open ProductLattice in
/-- The Aeneas-generated `CapabilityLattice.top` is the lattice ⊤. -/
theorem lattice_top_eq_top :
    nucleus_ifc_kernel.CapabilityLattice.top = .ok ⊤ := by
  show _ = Result.ok latticeTop
  simp [nucleus_ifc_kernel.CapabilityLattice.top, latticeTop]
  constructor <;> rfl

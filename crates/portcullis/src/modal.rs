...
//!
//! ```rust
//! use portcullis::modal::{ModalPermissions, ModalContext};
    ...
    fn necessity(&self) -> Self {
        ...
        for op in &self.obligations.approvals {
            match op {
                ...
                Operation::SpawnAgent => result.capabilities.spawn_agent = CapabilityLevel::Never,
            }
        }
...
pub fn all_capability_modals(perms: &PermissionLattice) -> Vec<CapabilityModal> {
    vec![
        ...
        CapabilityModal::from_perms(perms, Operation::SpawnAgent),
    ]
}

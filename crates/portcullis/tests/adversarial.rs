...
fn effective_permissions_detect_tampering() {
    use portcullis::EffectivePermissions;
...
fn extreme_time_bounds_handled() {
    ...
    let lattice = portcullis::TimeLattice::between(ancient, far_future);

    // Should be valid now
    assert!(lattice.is_valid());

    // Meet with default should work
    let default = portcullis::TimeLattice::default();
...
fn isolation_cel_constraint_evaluation() {
    use portcullis::constraint::{Constraint, PolicyContext};
    use portcullis::isolation::IsolationLattice;

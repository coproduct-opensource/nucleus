...
//!
//! **Without portcullis**: Every agent is in its own sandbox. Scout's
...
//!
//! **With portcullis**: The meet operation ensures Scout can only delegate
...
///
/// With portcullis: Scout can only delegate permissions it HAS. Since
...
fn scenario_3a_budget_cannot_exceed_parent() {
    ...
    let err = parent
        ...
        .unwrap_err();
    assert!(
        matches!(err, portcullis::DelegationError::InsufficientBudget { .. }),

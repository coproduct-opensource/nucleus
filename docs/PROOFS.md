...
| **Kani bounded model checking** | 122 `#[kani::proof]` harnesses across crates (census: `scripts/formal-numbers.sh`) | Overflow/panic-freedom and bounded functional properties on the real Rust (e.g. welfare-overflow). Bounded, not unbounded — see §4. |

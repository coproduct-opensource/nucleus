...
|-----------|-------|-----------------|
| **Golden-vector seal (G3)** | `crates/nucleus-econ-kernels/tests/golden/*.json` + readers | One JSON single-source pins settlement / commons / VCG outputs across **four** implementations: Lean `decide`-checks (`Nucleus/Golden.lean`, settlement), the **Rust** kernel reader (`tests/golden.rs`, settlement + commons + VCG), and — as they land — the Foundry/Solidity and WASM (`@coproduct/verify`) readers. Editing any one implementation so it diverges from the bytes turns CI red. |

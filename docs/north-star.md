...
| --- | --- | --- | --- | --- |
| F1 | "any model" — a second vendor, an open-weight model behind a generic client, and a non-LLM worker each run in-guest and every published metric is reported per model (`models_covered` grow-only) — tracked in #2699 | NOT-YET | `crates/nucleus-tool-proxy/tests/red_team_harness.rs#LLM_API_URL`, `.github/workflows/red-team-agent.yml`, `reviews/north-star-frontier/SYNTHESIS.md` (N-26, Q-4) | — |
| F2 | "any model" — the agent process holds no ambient host authority in any shipped tier: the boundary is the pod, not one CLI's disallow-list and hook contract — tracked in #2696 | NOT-YET | `crates/nucleus-cli/src/constants.rs#DISALLOWED_BUILTIN_TOOLS`, `crates/nucleus-spec/src/lib.rs#pub struct WorkloadSpec`, `reviews/north-star-frontier/SYNTHESIS.md` (N-01, Q-1) | — |
| F3 | "as much useful real-world work" — the default exec tool round-trips over the wire: the MCP face and the proxy compile against ONE shared request type (`nucleus-api-types`), a test pins the advertised tool schema to that type's fields, and the legacy `command` spelling resolves to the same argv as `args` (no shell) | TESTED | `crates/nucleus-api-types/src/lib.rs#pub struct RunRequest`, `crates/nucleus-mcp/src/main.rs#run_tool_schema_and_proxy_wire_type_agree`, `crates/nucleus-api-types/src/lib.rs#the_alias_is_argv_equivalent_and_cannot_smuggle_an_interpreter` | `.github/workflows/ci.yml` |
| F4 | "as much useful real-world work" — a delegation matrix (profile × operation × tier) is published and grow-only, each row showing its denied variant refusing by policy — tracked in #2698, #2704 | NOT-YET | `docs/perf/RUBRIC-LEDGER.md`, `crates/nucleus-cli/src/profiles.rs`, `reviews/north-star-frontier/SYNTHESIS.md` (N-28) | — |
| F5 | "as much useful real-world work" — credentialed effects (git push, pull request, cloud API) are reachable from the agent through the broker with method/path scoping and a receipt — tracked in #2698 | NOT-YET | `crates/nucleus-tool-proxy/src/egress.rs`, `crates/nucleus-spec/src/lib.rs#CredentialedEgressSpec`, `reviews/north-star-frontier/SYNTHESIS.md` (N-04, Q-3) | — |
| F6 | "as its principal is willing to authorize" — an approval is consulted on the decision path, consumed exactly once, bounded in uses, refused over mTLS unless the identity is on the approver roster, and recorded under the key or SPIFFE ID that verified it (all landed, unit-tested); what keeps this NOT-YET: the HMAC tiers still record `Unknown` (a shared secret names nobody), and the approval is not yet bound to the action's argument hash — tracked in #2697 | NOT-YET | `crates/nucleus-tool-proxy/src/main.rs#approved via /v1/approve`, `crates/nucleus-tool-proxy/src/main.rs#bounded_approval_count`, `crates/nucleus-tool-proxy/src/policy.rs#pub fn is_approver`, `crates/nucleus-tool-proxy/src/auth.rs#approver_key`, `crates/portcullis/src/verdict_sink.rs#SignedBy`, `reviews/north-star-frontier/SYNTHESIS.md` (N-05, N-06, Q-2) | — |
| F7 | "as its principal is willing to authorize" — the profile's path allowlist is carried as the certificate's `SinkScope` on every mint path (node delegation, re-rooting, bootstrap, `nucleus token delegate`), and ONE glob-subsumption primitive — gatehouse's `subsetGlob` shape, its golden cases ported — decides containment for certificate scopes, delegation scopes, and the path lattice's meet and order (a narrower child no longer meets its parent to NOTHING); what stays open: hosts and git refs have no profile dimension yet, and the session task token's `allowed_paths` is still empty by design (F4 tracks the live-pod half) | TESTED | `crates/portcullis-core/src/glob.rs#pub fn glob_subsumes`, `crates/portcullis/src/certificate.rs#pub fn from_lattice`, `crates/portcullis/src/path.rs#glob_subsumes`, `crates/nucleus-node/src/pod_authority.rs#SinkScope::from_lattice`, `crates/portcullis-core/src/delegation.rs#proof_glob_subsumes_respects_the_component_boundary`, `crates/portcullis/tests/proptest_lattice.rs#path_meet_is_below_both_sides` | `.github/workflows/kani-spawn-boundary.yml` |
| F8 | "as its principal is willing to authorize" — revocation reaches every issued offline-verifiable carrier: a revoked prefix fails every descendant — tracked in #2697 | NOT-YET | `crates/portcullis/src/certificate.rs#pub fn verify_certificate`, `reviews/north-star-frontier/SYNTHESIS.md` (N-07, Q-2) | — |
...
| F11 | "structurally incapable of exceeding that authorization" — the confidentiality sub-ledger (C1–C9, below) carries no NOT-YET row — tracked in #2706 | NOT-YET | `scripts/north-star-ledger-ratchet.txt#NOT_YET`, `docs/cross-pod-view.md` (C2), `crates/nucleus-identity/src/attestation.rs` (C9) | — |
...
| F13 | "structurally incapable of exceeding that authorization" — Tier 1 (`nucleus run --local`) is a principal boundary: distinct uid, no shared secret readable by the agent, Landlock/seccomp on children — tracked in #2696 (same Landlock/seccomp mechanism, applied to the host tier) | NOT-YET | `crates/nucleus/src/hardening.rs`, `reviews/north-star-frontier/SYNTHESIS.md` (N-19) | — |
| F14 | "continuously expand the frontier" — utility under authorization is measured per canonical profile under that profile's REAL lattice (`nucleus-flow-replay --frontier-corpus` → `frontier.json`, embedded in the exemplar scoreboard): `allowed_share_permille` is higher-is-better, and `corpus_steps_GUARD` and each profile's `denied_at_exfil_vector_GUARD` may not fall, so the share cannot be bought by shrinking the corpus or opening an exfil sink; pinned in both directions in `scripts/exemplar-baseline.json` | RATCHETED | `crates/nucleus-flow-replay/src/lib.rs#pub fn frontier`, `crates/nucleus-flow-replay/src/lib.rs#allowed_share_permille`, `crates/xtask/src/scoreboard.rs#allowed_share_permille`, `scripts/exemplar-baseline.json`, `crates/nucleus-flow-replay/corpus/corpus.jsonl` | `scripts/exemplar-scoreboard.sh` |
| F15 | "continuously expand the frontier" — the exemplar scoreboard is declared a required context (`ci/required-checks.txt`, PINNED grow-only; `ci-spec live-parity` reconciles branch protection), reported in the merge queue and unfiltered, and `cargo xtask scoreboard-ratchet` refuses an improvement that is not re-pinned in the same change and any pinned metric that vanished — so a baseline can carry no slack | DECIDED | `ci/required-checks.txt#exemplar metrics ratchet`, `.github/workflows/exemplar-scoreboard.yml#merge_group`, `crates/xtask/src/scoreboard.rs#not moved in the same change`, `crates/xtask/src/scoreboard.rs#vanished from the current scoreboard` | `.github/workflows/exemplar-scoreboard.yml` |
...
| Metric | Count | Where |
|---|---|---|
| Kani BMC harnesses | 119 | portcullis 68, portcullis-core 27, ck-kernel 17, nucleus-ifc-kernel 6, nucleus-econ-kernels 1 (`scripts/formal-numbers.sh`) |
...
|---|---|---|---|
| **Current** | 119 | ~277 theorems | Lattice laws, BMC safety, IFC noninterference, budget conservation |

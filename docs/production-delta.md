...
| `Sandbox` write mediation | Partial | `Sandbox::{write,write_approved}` took the discharge bundle as `_proof` and never read it, so any bundle authorized a write — the same confused deputy fixed in `NucleusRuntime`. Both now take an owned `Authority` and spend it against `(WriteFiles, WorkspaceWrite)`. **The wider gap is open**: of the 25 public I/O methods on `Sandbox`, these 2 are the only ones that require a discharge at all. The other 23 pass `DecisionToken` + capability-lattice checks but none of the eight obligations — `create`, `create_dir`, `create_dir_all`, `remove_file`, `remove_dir`, `open_with` and their `_approved` variants included, so `remove_file` deletes with no discharge. (`read_to_string_for_search` takes not even a `DecisionToken`.) |

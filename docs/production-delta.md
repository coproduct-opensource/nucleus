...
| Effect receipts (witness) | Partial | Every mediation decision through `PolicyEnforced` appends a leaf to an RFC 6962 Merkle log naming the `(Operation, SinkClass)` and whether it was allowed, denied by capability, or denied by scope. Supports **inclusion** proofs (`O(log n)` — an auditor checks one action without receiving the log) and **consistency** proofs (the log only grew; nothing was rewritten behind them), plus an Ed25519-signed checkpoint. Append-only by construction — no `remove`, no `truncate`, no access to the inner `Vec`. **Nothing is published**, so a *forked* log — two validly signed checkpoints of the same size with different roots — is not detectable without a witness or third-party anchor; Rekor is the escalation path and is not done. The signing key is caller-supplied and not bound to a measured binary. `ShellEffect::run_argv`, `AsyncShellSpawnEffect::run_argv_async` and `NetEffect::fetch` spend below this layer and are not witnessed. |

...
| Enforcement completeness (#1216) | Partial | Restated: the gap is inside the effect layer, not at scattered call sites (the crate the old count named no longer exists). **All 13 effect-trait methods now require an obligation token**, up from 3. **10 take an `Authority` by value** — scoped, and one-shot by move semantics, so replay is a compile error: `FileEffect::{read,write,append,glob}`, `WebEffect::{fetch,search}`, `ShellEffect::run`, `GitEffect::{commit,push}`, `AgentSpawnEffect::spawn`, plus the async mirrors. **3 still take `&DischargedBundle`** — `ShellEffect::run_argv`, `AsyncShellSpawnEffect::run_argv_async`, `NetEffect::fetch` — so a correctly-scoped bundle can still be replayed on the structured-spawn and net-egress paths. Those three have callers in `nucleus` and `nucleus-tool-proxy`; converting them is the remaining B2 work. |

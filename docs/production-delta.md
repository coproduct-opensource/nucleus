...
| Discharge scope binding | Done | Every mediated `NucleusRuntime` method checks the bundle's `(Operation, SinkClass)` against its own, and `write_file` re-checks the path allowlist. Previously the bundle was accepted unread, so a read bundle authorized a write and could redirect it outside `allowed_write_paths`. |
| Enforcement completeness (#1216) | Open | Restated: the gap is inside the effect layer, not at scattered call sites (the crate the old count named no longer exists). 10 of 13 effect-trait methods — including `FileEffect::write` — take no `DischargedBundle` and are gated only by the coarse capability lattice, not the eight obligations. The 3 that do take one receive it by reference, so a correctly-scoped bundle can be replayed. |

...
| Enforcement completeness (#1216) | Partial | Restated: the gap is inside the effect layer, not at scattered call sites (the crate the old count named no longer exists). `FileEffect` (`read`/`write`/`append`/`glob`, plus the async mirror) now takes an `Authority` **by value** — scoped, and one-shot by move semantics, so replay is a compile error. **6 of 13 methods remain unmediated**: `WebEffect::{fetch,search}`, `ShellEffect::run`, `GitEffect::{commit,push}`, `AgentSpawnEffect::spawn` still take no obligation token and are gated only by the coarse lattice. The 3 structured-spawn/net methods take a `&DischargedBundle`, so a correctly-scoped bundle can still be replayed there. |

...
| `Sandbox` write mediation | Partial | `Sandbox::{write,write_approved}` took the discharge bundle as `_proof` and never read it, so any bundle authorized a write — the same confused deputy fixed in `NucleusRuntime`. Both now take an owned `Authority` and spend it against `(WriteFiles, WorkspaceWrite)`. **The wider gap is open**: of 25 public `Sandbox` methods, 17 mutate and only these 2 require a discharge at all. `open_with`, `create`, `create_dir`, `create_dir_all`, `remove_file`, `remove_dir`, `open_dir` and their `_approved` variants take no obligation token — they pass `DecisionToken` + capability-lattice checks but none of the eight obligations. `remove_file` deletes with no discharge. |

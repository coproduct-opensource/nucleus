...
# --- Tier B: Mayer-Vietoris subadditivity on composed prompts ---
md(
    "## 12. Tier B theory-selection test — Mayer-Vietoris subadditivity\n",
    "\n",
    "Tier A (Euler) is a **static** structural test. Tier B tests the **compositional** ",
    "structure: when two prompts are concatenated, the rank H¹ of the composite is ",
    "bounded by the component ranks plus a correction term — the Grothendieck ",
    "spectral sequence analog of Mayer-Vietoris.\n",
    "\n",
    "Our theory (PR #1573 `spectral_sequence_cost_bound`) predicts:\n",
    "\n",
    "> rank H¹(P₁ ++ P₂) ≤ rank H¹(P₁) + rank H¹(P₂) + rank H²(P₁ ++ P₂)\n",
    "\n",
    "Attention-anomaly and MI-based theories predict **no such specific form**. They ",
    "might expect some subadditivity, but not with the specific `+ h²Obstruction` ",
    "correction term. If our inequality holds generically and tightly — and particularly ",
    "if the gap (h² correction) correlates with *link complexity* between prompts — ",
    "our theory is selected.\n",
    "\n",
    "**Vindication thresholds**:\n",
    "- **Soundness**: 100% of composition pairs satisfy the inequality. Violation would ",
    "  mean our theorem is false on attention sheaves (or our construction is wrong).\n",
    "- **Informativeness**: the correction term `h²(composite)` is non-zero for ≥ 30% of ",
    "  pairs. If always zero, the inequality degenerates to plain subadditivity and ",
    "  Tier B is not discriminating.\n",
    "- **Correlation**: when one of P₁, P₂ is injected, the composite `rank H²` is ",
    "  systematically larger than benign-benign compositions. This tests whether link ",
    "  complexity tracks with injection presence.",
)

code(
    "def compose_prompts(p1: str, p2: str) -> str:\n",
    "    '''Concatenate with separator. Mirrors prompt-chaining in LLM pipelines.'''\n",
    "    return p1.strip() + '\\n\\n' + p2.strip()\n",
    "\n",
    "def subadditivity_gap(p1: str, p2: str) -> dict:\n",
    "    '''Measure the Mayer-Vietoris inequality:\n",
    "      rank H¹(P₁++P₂)  vs  rank H¹(P₁) + rank H¹(P₂) + rank H²(P₁++P₂).'''\n",
    "    a1 = extract_attention(p1)\n",
    "    a2 = extract_attention(p2)\n",
    "    a12 = extract_attention(compose_prompts(p1, p2))\n",
    "    h1_1 = reduced_cech_h1(a1)\n",
    "    h1_2 = reduced_cech_h1(a2)\n",
    "    h1_12 = reduced_cech_h1(a12)\n",
    "    h2_12 = reduced_cech_h2(a12)\n",
    "    bound = h1_1 + h1_2 + h2_12\n",
    "    return {\n",
    "        'h1_1': h1_1, 'h1_2': h1_2, 'h1_composite': h1_12,\n",
    "        'h2_composite': h2_12, 'bound': bound,\n",
    "        'holds': h1_12 <= bound, 'slack': bound - h1_12,\n",
    "    }\n",
    "\n",
    "# Run over paired benchmark prompts\n",
    "from itertools import combinations\n",
    "pairs = list(combinations(BENIGN[:3], 2)) + [(b, i) for b in BENIGN[:2] for i in INJECTED[:2]]\n",
    "results = [subadditivity_gap(p1, p2) for p1, p2 in tqdm(pairs, desc='composing')]\n",
    "n_holds = sum(r['holds'] for r in results)\n",
    "print(f'Tier B soundness: {n_holds}/{len(results)} pairs satisfy the inequality')\n",
    "# Non-zero h² rate\n",
    "n_h2_nonzero = sum(r['h2_composite'] > 0 for r in results)\n",
    "print(f'Informativeness: {n_h2_nonzero}/{len(results)} composites have rank H² > 0')\n",
    "# Benign-benign vs benign-injected split\n",
    "bb = [r['h2_composite'] for r, (p1, p2) in zip(results, pairs) if p1 in BENIGN and p2 in BENIGN]\n",
    "bi = [r['h2_composite'] for r, (p1, p2) in zip(results, pairs) if p1 in BENIGN and p2 in INJECTED]\n",
    "if bb and bi:\n",
    "    print(f'h² mean (benign-benign): {np.mean(bb):.2f}')\n",
    "    print(f'h² mean (benign-injected): {np.mean(bi):.2f}')\n",
    "    print('Positive gap → link-complexity correlates with injection presence.')\n",
)

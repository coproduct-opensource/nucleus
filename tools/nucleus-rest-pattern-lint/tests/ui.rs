...
// processes compiletest spawns. The siblings fail identically on the same host, so
// this is pre-existing and not specific to this crate.
//
...
// and the ENFORCING check is `scripts/check-rest-pattern-dylint.sh --self-test`,
// which perturbs the real delegation path rather than a fixture.
#[test]
